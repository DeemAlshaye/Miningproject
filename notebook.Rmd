...
###checking for imbalance class lable

```{r}
barplot(table(ds_salaries$salary_in_usd_disc),
        ylab = "Frequency",
        xlab = "salary")
```
...
train_indices_1 <- createDataPartition(ds_salaries$salary_in_usd_disc, p = 0.7, list = FALSE, times = 1)

# Create the training and test sets
data_train70 <- ds_salaries[train_indices_1, ]
data_test70 <- ds_salaries[-train_indices_1, ]
...
# The train set has 2491 rows while the test set has 106 rows.
...
library(printr)
set.seed(123)
train.indices <- sample(1:nrow(ds_salaries), 200)
data.train <- ds_salaries[train.indices, ]
data.test <- ds_salaries[-train.indices, ]
```
...
```{r}
table(results, data.test$salary_in_usd_disc)
```
...
ind <- sample(2, nrow(ds_salaries), replace = TRUE, prob = c(0.8, 0.2))
train_data <- ds_salaries[ind == 1, ]
test_data <- ds_salaries[ind == 2, ]
```

# Define the formula

```{r}
myFormula <- salary_in_usd_disc ~ work_year + experience_level + employment_type + job_title + company_location + company_size + remote_ratio

dt_model <- rpart(myFormula, data = train_data, method = "class", parms = list(split = "information"))
...
rpart.plot(dt_model,extra = 100,type = 4)
```
...
trainPred <- predict(dt_model, newdata = train_data, type = "class")
table(trainPred, train_data$salary_in_usd_disc)
```
...
testPred <- predict(dt_model, newdata = test_data, type = "class")
table(testPred, test_data$salary_in_usd_disc)
```

```{r}
result<-table(testPred, test_data$salary_in_usd_disc)
co_result <- confusionMatrix(result)
print(co_result)
```
