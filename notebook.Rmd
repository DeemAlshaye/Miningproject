...
we see that the high salary in train data is higher than the test data
...
```{r}
table_mat <- table(data_test$salary_in_usd_disc, predict_unseen)
table_mat
```
...
data.train <- create_train_test(ds_salaries, 0.8, train = TRUE )
data.test <- create_train_test(ds_salaries, 0.8, train = FALSE )
```

Train decision tree

```{r}
model <- C5.0(salary_in_usd_disc ~., data=data.train)
```

Test

```{r}
results <- predict(model, data.test, type="class")
...
```{r}
library(caret)
result1<- table(results, data.test$salary_in_usd_disc)
...
classify_dataset <- function(dataset, class_label) {
  ...
  train_indices <- sample(1:nrow(dataset), 0.8 * nrow(dataset))  # 80% train, 20% test
  train_data <- dataset[train_indices, ]
  test_data <- dataset[-train_indices, ]

  # Create the decision tree model using information gain
  decision_tree <- rpart(formula = as.formula(paste(class_label, "~ .")), data = train_data, method = "class", control = rpart.control(cp = 0))
 ...
 # Predict on the test set
  ...
  confusion_matrix <- confusionMatrix(predictions, test_data[, class_label])
...
# 3.Split the Dataset into two subsets: Training(90%) and Testing(10%):

```{r}
set.seed(65788)

ind3 <- sample(2, nrow(ds_salaries), replace = TRUE, prob = c(0.9, 0.1))

data_train90 <- ds_salaries[ind3 == 1, ]
data_test90 <- ds_salaries[ind3 == 2, ]
...
# The train dataset has 3167 rows while the test dataset has 380 rows.
...
library(rpart)
library(rpart.plot)
fit3 <- rpart(salary_in_usd_disc~., data = data_train90, method = 'class')
rpart.plot(fit3,type = 4,)
```
# rpart() function uses the Gini impurity measure to split the note.

```{r}
predict_unseen90 <-predict(fit3, data_test90, type = 'class')
```

```{r}
table_mat90 <- table(data_test90$salary_in_usd_disc, predict_unseen90)
table_mat90
```

```{r}
accuracy_Test90 <- sum(diag(table_mat90)) / sum(table_mat90)
```

```{r}
print(paste('Accuracy for test', accuracy_Test90))
```


```{r}
result90_gini<-table(predict_unseen90, data_test90$salary_in_usd_disc)
co_result90_gini <- confusionMatrix(result90_gini)
print(co_result90_gini)
```
...
```{r}
model90 <- C5.0(salary_in_usd_disc ~., data=data_train90)
```

Test

```{r}
results <- predict(object=model90, newdata=data_test90, type="class")
...
```{r}
table(results, data_test90$salary_in_usd_disc)
```
...
classify_dataset <- function(dataset, class_label) 
  ...
  # Create the decision tree model using information gain
  decision_tree <- rpart(formula = as.formula(paste(class_label, "~ .")), data = data_train90, method = "class", control = rpart.control(cp = 0))
 ...
 # Predict on the test set
  predictions <- predict(decision_tree, newdata = data_test90, type = "class")
  
  # Create confusion matrix
  confusion_matrix <- confusionMatrix(predictions, data_test90[, class_label])
